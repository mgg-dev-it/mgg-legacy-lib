...
import hu.mgx.draw.*;
import hu.mgx.report.*;

public class PreviewReportPanelNew extends CommonPanel implements ActionListener, ChangeListener
{
    ...
    private final int iMinMagnification = 1;
    private final int iMaxMagnification = 200;

    public PreviewReportPanelNew(SwingApp app)
    {
    ...
    //    }
    public PreviewReportPanelNew(SwingApp app, PageFormat pageFormat, Dimension dimPreferred)
    {
        super();
        this.app = app;
        ...
        addToGrid(jRadioButtonPortrait = AppUtils.createRadioButton(app.getLanguageString(DefaultLanguageConstants.STRING_CAPTION_PRPN_PORTRAIT), "Portrait", this), 1, 0, 1, 1, 0.0, 0.0, GridBagConstraints.NONE);
        addToGrid(jRadioButtonLandscape = AppUtils.createRadioButton(app.getLanguageString(DefaultLanguageConstants.STRING_CAPTION_PRPN_LANDSCAPE), "Landscape", this), 1, 1, 1, 1, 0.0, 0.0, GridBagConstraints.NONE);
        addToGrid(new JLabel(app.getLanguageString(DefaultLanguageConstants.STRING_CAPTION_PRPN_MAGNIFY)), 1, 2, 1, 1, 0.0, 0.0, GridBagConstraints.NONE);
        addToGrid(jSpinnerRate, 1, 3, 1, 1, 0.0, 0.0, GridBagConstraints.NONE);
        addToGrid(new JLabel(app.getLanguageString(DefaultLanguageConstants.STRING_CAPTION_PRPN_PAGE)), 1, 4, 1, 1, 0.0, 0.0, GridBagConstraints.NONE);
        ...
        addToGrid(jButtonPrint = AppUtils.createButton(app.getLanguageString(DefaultLanguageConstants.STRING_CAPTION_PRPN_PRINT), "Print", this), 1, 7, 1, 1, 0.0, 0.0, GridBagConstraints.NONE);
        addToGrid(jButtonPageSetup = AppUtils.createButton(app.getLanguageString(DefaultLanguageConstants.STRING_CAPTION_PRPN_PAGE_SETUP), "PageSetup", this), 1, 8, 1, 1, 0.0, 0.0, GridBagConstraints.NONE);
    ...
    public void setMagnification(int iMagnification)
    {
        if (iMagnification < iMinMagnification)
        {
            iMagnification = iMinMagnification;
        }
        if (iMagnification > iMaxMagnification)
        {
            iMagnification = iMaxMagnification;
        }
        spinnerNumberModelRate.setValue(new java.lang.Integer(iMagnification));
    }

    public int getMagnification()
    {
        return (((Integer) spinnerNumberModelRate.getValue()).intValue());
    }
    ...
    private void printReport()
    {
        ...
        if (pageFormat.getOrientation() == PageFormat.PORTRAIT)
        {
            aset.add(new MediaPrintableArea(new Double(pageFormat.getImageableX() / 72.0).floatValue(), new Double(pageFormat.getImageableY() / 72.0).floatValue(), new Double(pageFormat.getImageableWidth() / 72.0).floatValue(), new Double(pageFormat.getImageableHeight() / 72.0).floatValue(), MediaPrintableArea.INCH));
        }
        if (pageFormat.getOrientation() == PageFormat.LANDSCAPE)
        {
            aset.add(new MediaPrintableArea(new Double(pageFormat.getImageableY() / 72.0).floatValue(), new Double(pageFormat.getImageableX() / 72.0).floatValue(), new Double(pageFormat.getImageableHeight() / 72.0).floatValue(), new Double(pageFormat.getImageableWidth() / 72.0).floatValue(), MediaPrintableArea.INCH));
        }
        if (pageFormat.getOrientation() == PageFormat.REVERSE_LANDSCAPE)
        {
            aset.add(new MediaPrintableArea(new Double(pageFormat.getImageableY() / 72.0).floatValue(), new Double(pageFormat.getImageableX() / 72.0).floatValue(), new Double(pageFormat.getImageableHeight() / 72.0).floatValue(), new Double(pageFormat.getImageableWidth() / 72.0).floatValue(), MediaPrintableArea.INCH));
