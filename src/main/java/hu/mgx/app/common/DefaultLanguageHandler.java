...
public class DefaultLanguageHandler implements LanguageHandlerInterface {
    ...
    private void init() {
        ...
        setLanguageString(DefaultLanguageConstants.STRING_MESSAGE_PASSWORD_CHANGE_UNSUCCESSFUL, "Jelszócsere nem sikerült.");
        ...
        setLanguageString(DefaultLanguageConstants.STRING_CAPTION_PRPN_LANDSCAPE, "Fekvõ");
        ...
        setLanguageStringXML_HU_RO("Mentés", "Salvare"); //OK
        setLanguageStringXML_HU_RO("Törlés", "Stergere"); //OK
        setLanguageStringXML_HU_RO("Mégsem", "Revocare"); //OK
        setLanguageStringXML_HU_RO("Frissítés", "Actualizare"); //OK
    }
...
//        vLanguages.addElement(pLanguage);
    ...
    public String getLanguageString(String sKey, String sDefaultValue) {
        if (!bSubstitute) //nem helyettesíti más nyelven, ha nem találja ...
        {
...
//        System.out.println(Integer.toString(iItemEnd));
        if (iItemStart < 0) {
            return (-1); //hibakezelés - nincs <item ...> elem
        }
        if (iItemStartClose < 0) {
            return (-1); //hibakezelés - nincs <item ...> lezáró elem
        }
        if (iItemEnd < 0) {
            return (-1); //hibakezelés - nincs </item> elem
        }
        ...
        if (iItemEnd <= iItemStart) {
            return (-1); //hibakezelés - itemEnd <= itemStart
        }
...
//        System.out.println(sItem);
    ...
    public void setLanguageStringXML(String sXML) {
        ...
        if (iKeyStart < 0) {
            return; //hibakezelés - nincs <key> elem
        }
        if (iKeyEnd < 0) {
            return; //hibakezelés - nincs </key> elem
        }
        ...
        if (iKeyEnd <= iKeyStart) {
            return; //hibakezelés - keyEnd <= keyStart
        }
        sKey = sXML.substring(iKeyStart, iKeyEnd);
//        System.out.println("Key=" + sKey);
        //szövegek a különbözõ nyelveken:
