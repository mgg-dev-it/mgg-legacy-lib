...
public class TableSorterNewStyle extends TableMap {
    ...
    boolean bPreviousAscending = false;
    MemoryTable memoryTable = null; //2008-02-26

    public TableSorterNewStyle() {
        indexes = new int[0]; // for consistency
    }

    public TableSorterNewStyle(TableModel model) {
        setModel(model);
    }

    //2008-02-26
    public TableSorterNewStyle(MemoryTable memoryTable) {
        setModel(memoryTable);
    ...
    public int compareRowsByColumn(int row1, int row2, int column) {
        ...
        } else if (type.getName().startsWith("[B")) //2008.11.24-tõl ExcelExportImport-ból
        {
            ...
            sTmp += " 00:00:00";
            try {
                o1 = new java.util.Date(sqlDateTimeFormat.parse(sTmp).getTime());
            } catch (java.text.ParseException pe) {
                o1 = null;
            }
            sTmp = new String((byte[]) o2);
            sTmp += " 00:00:00";
            try {
                o2 = new java.util.Date(sqlDateTimeFormat.parse(sTmp).getTime());
            } catch (java.text.ParseException pe) {
                o2 = null;
            }
            Date d1 = (Date) o1; //data.getValueAt(row1, column);
            long n1 = d1.getTime();
            Date d2 = (Date) o2; //data.getValueAt(row2, column);
            long n2 = d2.getTime();
    ...
    public void sortByColumns(Vector<Integer> v, boolean ascending) {
        this.ascending = ascending;
        sortingColumns.removeAllElements();
        for (int i = 0; i < v.size(); i++) {
            sortingColumns.addElement(v.elementAt(i));
        }
        sort(this);
        super.tableChanged(new TableModelEvent(this));
    }
    ...
    public void addMouseListenerToHeaderInTable(JTable table) {
        final TableSorterNewStyle sorter = this;
        ...
        MouseAdapter listMouseListener = new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (e.getClickCount() == 1 && column != -1) {
                    if (memoryTable != null) {
                        if (!memoryTable.isColumnSortable(column)) {
                            return;
                        }
                    }
                    ...
                    if (iPreviousColumn == column) {
                        ascending = !bPreviousAscending;
                    }
                    if (shiftPressed != 0) {
                        ascending = false;
                    }
                    ...
                    int iPrevHeaderColumn = iPreviousColumn;

                    iPreviousColumn = column;
                    bPreviousAscending = ascending;

                    //2008-02-01
                    int iOrder = HeaderRenderer.ORDER_NONE;
                    if (ascending) {
                        iOrder = HeaderRenderer.ORDER_ASC;
                    }
                    if (!ascending) {
                        iOrder = HeaderRenderer.ORDER_DESC;
                    }
                    if (iPrevHeaderColumn > -1) {
                        tableView.getTableHeader().getColumnModel().getColumn(iPrevHeaderColumn).setHeaderRenderer(new HeaderOrderRenderer(HeaderOrderRenderer.ORDER_NONE));
                    }
                    tableView.getTableHeader().getColumnModel().getColumn(column).setHeaderRenderer(new HeaderOrderRenderer(iOrder));
                }
            }
        };
