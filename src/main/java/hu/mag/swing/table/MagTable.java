...
//                actionSelectNextColumnCell.actionPerformed(e);
        ...
        sXMLConfig += "        </languageitem>";
        sXMLConfig += "        <languageitem key='MagTable Fejlesztői információ'>";
        sXMLConfig += "            <translation lang='hu'>Fejlesztői információ</translation>";
        ...
        sXMLConfig += "            <translation lang='hu'>Szűrés</translation>";
...
//        swingAppInterface.addLanguageXML(sXMLConfig, "ISO-8859-2");
        ...
        if (swingAppInterface.getDateTimeVersion().equals(DEVELOPER_VERSION)) {
            miDeveloperInfo = new JMenuItem(swingAppInterface.getLanguageString("MagTable Fejlesztői információ"));
...
//        }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (e.getClickCount() == 1) {
            //MaG 2016.10.12. b�v�tve jobb eg�rgombbal �s pontos oszlop inform�ci�val fireMagTableEventClick(this.getSelectedRow());
        ...
        try {
            ...
            if (keyChanged(iRow)) {
                ...
                if (iKeyColumn > 0) {
                    ...
                    rs = ps.executeQuery();
                    if (rs.next()) {
                        //@todo task: azonos�t�kat beletenni az �zenetbe
                        //@todo wish: az els� �rintett mez�re helyezni a f�kuszt
                        AppUtils.messageBox(parentFrame, "Ilyen azonos�t�j� rekord m�r l�tezik!");
                        return (false);
                    }
                }
            }
            ...
            for (int i = 0; i < getColumnCount(); i++) {
                ...
                if (magTableModel.getResultSetMetaData().isNullable(i + 1) == ResultSetMetaData.columnNoNulls) {
                    ...
                    if (!magTableModel.isIdentityColumn(i) && !magTableModel.isVirtualColumn(i)) {
                        if (this.getValueAt(iRow, i) == null) {
                            AppUtils.messageBox(parentFrame, magTableModel.getColumnName(i) + " mez� nem lehet �res!");
                ...
                if (magTableModel.isMandatoryColumn(i)) {
                    if (StringUtils.isNull(this.getValueAt(iRow, i), "").equalsIgnoreCase("")) {
                        AppUtils.messageBox(parentFrame, magTableModel.getColumnName(i) + " mez�t k�telez� kit�lteni!");
                ...
                if (!magTableModel.getAllowedCharacters(i).equalsIgnoreCase("")) {
                    if (!StringUtils.containsAllowedCharactersOrEmpty(StringUtils.isNull(this.getValueAt(iRow, i), ""), magTableModel.getAllowedCharacters(i))) {
                        AppUtils.messageBox(parentFrame, magTableModel.getColumnName(i) + " mez� nem megengedett karaktert tartalmaz! (Megengedett karakterek: " + magTableModel.getAllowedCharacters(i) + ")");
                        setColumnSelectionInterval(i, i); //MaG 2018.10.13.
                        return (false);
                    }
                }
...
//                }
                ...
                if (magTableModel.getResultSetMetaData().getPrecision(i + 1) > 0 && magTableModel.getResultSetMetaData().getColumnClassName(i + 1).equals("java.lang.String") && StringUtils.isNull(this.getValueAt(iRow, i), "").length() > magTableModel.getResultSetMetaData().getPrecision(i + 1)) {
                    AppUtils.messageBox(parentFrame, magTableModel.getColumnName(i) + " mez� tartalma legfeljebb " + magTableModel.getResultSetMetaData().getPrecision(i + 1) + " karakter hossz� lehet");
                    setColumnSelectionInterval(i, i); //MaG 2018.10.13.
                    return (false);
                }
    ...
    //@todo translation: Figyelem! T�rli a rekordot? / azonos�t�j� rekord nem l�tezik / rekord t�rl�sre ker�lt / rekord t�rl�sre nem siker�lt
    private boolean deleteRecord(int iRow) {
        ...
        if (magTableModel.getRowStatus(convertRowIndexToModel(iRow)) == MagTableModel.ROW_STATUS_NEW_MODIFIED) {
            if (!AppUtils.yesNoQuestion(parentFrame, "Figyelem!", "T�rli az �j rekordot?", swingAppInterface)) {
                return (false);
            }
            this.getMagTableModel().removeRow(convertRowIndexToModel(iRow));
            int iNextRow = iRow - 1;
...
//        int iModelRow = this.convertRowIndexToModel(iRow);
        ...
        if (!AppUtils.yesNoQuestion(parentFrame, "Figyelem!", "T�rli (" + sRecordIdentifier + ") rekordot?", swingAppInterface)) {
            return (false);
        }
        try {
            sTableName = magTableModel.getTableName();
            ...
            rs = ps.executeQuery();
            if (!rs.next()) {
                AppUtils.messageBox(parentFrame, "(" + sRecordIdentifier + ") azonos�t�j� rekord nem l�tezik!");
                return (false);
            }
            rs.close();
            ps.close();
            ...
            if (iResult == 1) {
                ...
                AppUtils.messageBox(parentFrame, "(" + sRecordIdentifier + ") rekord t�rl�sre ker�lt.");
                //MaG 2018.05.07. fireMagTableEventRowDeleted(iRow);
                return (true);
            } else {
                AppUtils.messageBox(parentFrame, "(" + sRecordIdentifier + ") rekord t�rl�se nem siker�lt (" + Integer.toString(iResult) + ")");
                this.requestFocus();
                return (false);
            }
        } catch (SQLException sqle) {
